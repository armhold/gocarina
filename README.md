...
Also, a caveat: this software expects game boards to be 640x1136 pixels, as that is the size generated by my phone.
Your mobile device may use a different board size, based on its screen. Gocarina automatically scales the boards
to the expected size, but I haven't tested it exhaustively with every mobile device; you might have more success
in adjusting the geometry values such as `LetterPressExpectedWidth`, than with scaling alone.
